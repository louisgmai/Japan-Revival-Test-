...
class MotionSystem {
    ...
    setupWhyNowTransition() {
        // 1. ARRIVAL: The circle glides down to the Why Now section
        gsap.to(this.circle, {
            scrollTrigger: {
                trigger: "#why-now",
                start: "top bottom",
                end: "top 20%",
                scrub: true, // Use strict scrub (no lag) to ensure exact positioning
                invalidateOnRefresh: true
            },
            x: () => this.getAnchorViewportCoords(this.anchors.whyNow).x,
            y: () => {
                const whyRect = document.querySelector('#why-now').getBoundingClientRect();
                const anchorRect = this.anchors.whyNow.getBoundingClientRect();
                const relativeY = anchorRect.top - whyRect.top;
                return (window.innerHeight * 0.2) + relativeY;
            },
            scale: 1.5,
            ease: "power2.inOut"
        });

        // 2. LOCK: The circle attaches natively to the page from top 20% to the absolute bottom
        gsap.to(this.circle, {
            scrollTrigger: {
                trigger: "#why-now",
                start: "top 20%",
            ...
            y: () => {
                ...
                const anchorRect = this.anchors.whyNow.getBoundingClientRect();
                const relativeY = anchorRect.top - whyRect.top;
                const baseY = (window.innerHeight * 0.2) + relativeY;

                // Scroll distance from the start point to max
                const startScroll = (whyRect.top + window.scrollY) - (window.innerHeight * 0.2);
                const maxScroll = ScrollTrigger.maxScroll(window);
                const distance = maxScroll - startScroll;

                return baseY - distance;
            },
            ease: "none" // 1:1 scroll mimicry MUST be linear (no ease)
        });
    ...
    setupGenericScrollReveal() {
        // All remaining scroll-reveal elements that are NOT in the manually animated top sections
        const genericReveals = document.querySelectorAll('section:not(#about):not(#for-who):not(#why-now) .scroll-reveal');
